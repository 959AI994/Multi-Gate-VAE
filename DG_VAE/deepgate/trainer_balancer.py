...
import os
from DG_VAE.deepgate.gradnorm import Balancer
import torch
...
class Trainer():
    def __init__(self,
        ...
        # 将权重列表转为字典格式（关键修改1）
        self.loss_weights = {
            'prob_loss': prob_rc_func_weight[0],
            'recon_loss': prob_rc_func_weight[1],
            'func_loss': prob_rc_func_weight[2]
        }
    ...
    def set_training_args(self, prob_rc_func_weight=[], lr=-1, lr_step=-1, device='null'):
        ...
        if len(prob_rc_func_weight) == 3:
            ...
            if new_weights != self.loss_weights:
                ...
                # self.balancer.reset_weights(new_weights)
        ...
        for epoch in range(num_epoch): 
            for phase in ['train', 'val']:
                ...
                for iter_id, batch in enumerate(dataset):
                    ...
                    if phase == 'train':
                        # 使用Balancer计算动态权重损失（关键修改6）
                        losses_dict = {
                            'recon_loss': loss_status['recon_loss'],
                    ...
                    if self.local_rank == 0:
                        ...
                        Bar.suffix += '|Recon: {:.4f} |ACC: {:.2f} |Prob: {:.4f} |Func: {:.4f} '.format(recon_loss_stats.avg, acc_stats.avg * 100,prob_loss_stats.avg,func_loss_stats.avg)
                ...
                if self.local_rank == 0:
                    self.logger.write('{}| Epoch: {:}/{:} |Recon: {:.4f} |ACC: {:.2f} |Prob: {:.4f} |Func: {:.4f}|Net: {:.2f}s\n'.format(
