...
class Model(nn.Module):
    ...
    def __init__(self, 
        ...
        self.aggr_xor_strc = TFMlpAggr(dim_hidden, dim_hidden)
        ...
        self.aggr_xor_func = TFMlpAggr(dim_hidden*2, dim_hidden)
    ...
    def forward(self, G):
        ...
        if self.enable_encode:
            hs = generate_hs_init(G, torch.zeros(num_nodes, self.dim_hidden), 
                                 self.dim_hidden, xmg=True).to(device)
        else:
            hs = torch.zeros(num_nodes, self.dim_hidden).to(device)
            
        hf = torch.zeros(num_nodes, self.dim_hidden).to(device)
        edge_index = G.xmg_edge_index.to(device)
        
        # 门类型掩码
        gate_masks = {
            'and': G.xmg_gate.squeeze(1) == 3,
            'not': G.xmg_gate.squeeze(1) == 2,
            'xor': G.xmg_gate.squeeze(1) == 5,
            'maj': G.xmg_gate.squeeze(1) == 1,
            'or': G.xmg_gate.squeeze(1) == 4
        }
        ...
        for _ in range(self.num_rounds):
            ...
            # 分层处理
            for level in range(1, max(G.xmg_forward_level).item()+1):
                layer_mask = G.xmg_forward_level == level
                
                # 统一处理各门类型
                for gate_type in ['and', 'not', 'xor', 'maj', 'or']:
                    mask = gate_masks[gate_type] & layer_mask
                    if not torch.any(mask):
                        continue
                        
                    l_nodes = G.xmg_forward_index[mask]
