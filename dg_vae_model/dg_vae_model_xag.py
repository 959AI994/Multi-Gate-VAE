    ...
    def forward(self, G):
        ...
        if self.enable_encode:
            hs = generate_hs_init(G, torch.zeros(num_nodes, self.dim_hidden), 
                                 self.dim_hidden, xag=True).to(device)
        else:
            hs = torch.zeros(num_nodes, self.dim_hidden).to(device)
            
        hf = torch.zeros(num_nodes, self.dim_hidden).to(device)
        edge_index = G.xag_edge_index.to(device)
        
        # 门类型掩码
        gate_masks = {
            'and': G.xag_gate.squeeze(1) == 3,
            'not': G.xag_gate.squeeze(1) == 2,
            'xor': G.xag_gate.squeeze(1) == 5
        }
        ...
        for _ in range(self.num_rounds):
            ...
            # 分层处理
            for level in range(1, max(G.xag_forward_level).item()+1):
                layer_mask = G.xag_forward_level == level
                
                # 统一处理各门类型
                for gate_type in ['and', 'not', 'xor']:
                    mask = gate_masks[gate_type] & layer_mask
                    if not torch.any(mask):
                        continue
                        
                    l_nodes = G.xag_forward_index[mask]
